...
import { bestiaStructure } from './bestia.structure';
import { bestiaEN } from './bestia.en';
import { bestiaKO } from './bestia.ko';
...
function buildOverviewContent(content: typeof bestiaEN.overview): React.ReactNode {
...
function buildBestiaContent(lang: 'en' | 'ko'): ProjectDetailContent {
  const content = lang === 'ko' ? bestiaKO : bestiaEN;
  const structure = bestiaStructure;
...
export function getBestiaContent(lang: 'en' | 'ko'): ProjectDetailContent {
  return buildBestiaContent(lang);
}

// Export default English content for backward compatibility
export const bestiaContent = getBestiaContent('en');
