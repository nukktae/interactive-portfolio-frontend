...
import {
  ...
  Cpu, Image as ImageIcon, GitBranch, Target, Eye, Settings, ExternalLink
} from 'lucide-react';
import {
  SiPython, SiPytorch, SiJupyter, SiNumpy, SiPandas, SiOpencv,
  SiGooglemaps, SiFigma, SiGithub, SiOpenai
} from 'react-icons/si';

export const encarTrack1Content: ProjectDetailContent = {
  slug: 'encar-track1',
  sections: [
    {
      id: 'overview',
    ...
    },
    {
      ...
      type: 'goals',
      goals: [
        { icon: Target, title: 'Accuracy Target', metric: '>95% validation accuracy achieved' },
        { icon: Brain, title: 'Model Diversity', metric: '15+ models across 3 architectures (EfficientNet, MobileNet, CLIP)' },
        { icon: TrendingUp, title: 'Ensemble Performance', metric: 'Dual ensemble combining CNN and vision-language models' },
        { icon: GitBranch, title: 'Cross-Validation', metric: '5-fold CV with 3 seeds per fold for robust evaluation' },
        { icon: BarChart3, title: 'Production Ready', metric: 'Complete evaluation pipeline with CSV output generation' },
        { icon: Award, title: 'Competition Ready', metric: 'Reproducible, well-documented solution meeting all requirements' }
      ]
    },
    {
      id: 'research',
    ...
    },
    {
      ...
      items: [
        ...
        {
          title: 'Training Pipeline Workflows',
          userFlows: [
            {
      ...
      items: [
        {
          ...
          content: {
            components: [
              { title: 'BaseModel (EfficientNet-B0)', description: 'Primary CNN architecture with ImageNet pre-training, optimized for small datasets with compound scaling', icon: Layers },
              { title: 'MobileNetModel (MobileNetV3-Large)', description: 'Lightweight complementary model providing ensemble diversity with different inductive bias', icon: Component },
              { title: 'CLIPZeroShotModel', description: 'Vision-language model for zero-shot classification using text-image similarity matching', icon: Sparkles },
              { title: 'CLIPFineTunedModel', description: 'Domain-adapted CLIP with custom classification head and two-phase training strategy', icon: Brain },
              { title: 'EncarDataset', description: 'Custom PyTorch dataset class with flexible transform application and automatic class detection', icon: Database }
            ]
          }
        },
        {
          ...
          content: {
            components: [
              { title: 'Cross-Validation Trainer', description: 'Automated K-fold CV training with multiple seeds, checkpoint management, and comprehensive logging', icon: Workflow },
              { title: 'CLIP Trainer', description: 'Specialized training pipeline for CLIP models with two-phase strategy and CLIP-specific preprocessing', icon: Settings },
              { title: 'Ensemble Evaluator', description: 'Production-ready evaluation system combining multiple models with TTA and weighted averaging', icon: TrendingUp },
              { title: 'Data Augmentation Pipeline', description: 'Epoch-based augmentation schedule with Mixup, CutMix, and geometric transforms', icon: ImageIcon },
              { title: 'Test-Time Augmentation', description: '8-variant TTA system with rotations, flips, and color jitter for robust inference', icon: Eye }
            ]
          }
        }
      ]
      ...
      categories: [
        {
          ...
          items: [
            ...
            { name: 'TorchVision', icon: SiPytorch },
            { name: 'OpenAI CLIP', icon: SiOpenai },
            { name: 'TIMM', icon: SiPytorch }
          ]
        },
        {
    ...
    },
    {
      ...
      type: 'links',
      links: [
        { label: 'GitHub Repository', url: 'https://github.com', icon: ExternalLink },
        { label: 'Project Documentation', url: 'https://github.com', icon: ExternalLink }
      ],
      figmaUrl: '',
      caseStudyUrl: ''
    }
